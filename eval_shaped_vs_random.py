...
def reward_from_success(raw_obs, step_cost=-0.1):
    r = step_cost
...
def run_policy(env, policy_name="ppo", model=None, episodes=50):
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            # shaped reward computed from strings
            #obs_text = str(obs)
            #next_obs_text = str(next_obs)
            #info_text = str(info)

            #r_shaped, events = cyber_event_reward(obs_text, next_obs_text, info_text)
            
            
            txt = str(info.get("raw_obs", "")) + " " + str(info.get("raw_info", ""))
            r_shaped, events = cyber_event_reward("", "", txt)
