...
def reward_from_success(info, step_cost=-0.1):
    """Shaping based on CybORG Results.success captured into info."""
    r = step_cost
...
def run_policy(env, policy_name="ppo", model=None, episodes=50):
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            r_shaped = reward_from_success(raw_obs, step_cost=-0.1)

            # optional: punish done episode slightly (rarely needed)
            # if terminated: r_shaped -= 1.0

            ep_shaped += r_shaped

            done = terminated or truncated

            # For event counts, use success only (stable)
            events = {"cred": False, "comp": False}
            
            if isinstance(raw_obs, dict):
                s = str(raw_obs.get("success", "")).lower()
                # You can label "false" as a negative outcome
                if "false" in s:
                    events["cred"] = True
