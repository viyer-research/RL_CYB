...
def run_policy(env, policy_name="ppo", model=None, episodes=50):
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            # ---- 1) base step shaping using success ----
            r_shaped = reward_from_success(info, step_cost=-0.1)

            # ---- 2) add event shaping from observation/info text ----
            txt = str(info.get("raw_obs", "")) + " " + str(info.get("raw_info", ""))
            r_evt, events = cyber_event_reward("", "", txt, step_cost=0.0)
            r_shaped += r_evt
            ...
            # ---- IMPORTANT: update done + obs so loop exits ----
            done = terminated or truncated
            obs = next_obs

            # Debug (only first episode first few steps)
